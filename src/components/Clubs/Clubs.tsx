...
const Clubs: React.FC = () => {
  ...
  const hasAppliedToClub = (clubId: string) => {
    return userApplications.some(app => app.clubId === clubId);
  };

  const handleJoinClub = (club: Club) => {
    console.log('Join club clicked for:', club.name, 'User role:', user?.role);
    
    if (!user) {
      alert('You must be logged in to apply to clubs.');
      return;
    }
    ...
    console.log('Opening application modal for:', club.name);
